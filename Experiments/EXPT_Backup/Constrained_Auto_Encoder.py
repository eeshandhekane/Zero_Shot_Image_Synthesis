...
# Define a class to hold an attribute capsule
class attributeCapsule(object) :
	"""
	Holds the capsule intended to specialized in the 
	"""
...
ENC_W2 = tf.Variable(tf.truncated_normal([HIDDEN_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2')
ENC_B2 = tf.Variable(tf.truncated_normal([LATENT_DIM], stddev = 0.1), name = 'ENC_B2')
SAM_W_mu = tf.Variable(tf.truncated_normal([LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu')
SAM_B_mu = tf.Variable(tf.truncated_normal([LATENT_DIM], stddev = 0.1), name = 'SAM_B_mu')
SAM_W_logstd = tf.Variable(tf.truncated_normal([LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd')
SAM_B_logstd = tf.Variable(tf.truncated_normal([LATENT_DIM], stddev = 0.1), name = 'SAM_B_logstd')
...
Constraint_T_L2 = tf.nn.l2_loss(Y_4_1 - Y_true)

...
# Decide the integer that we want to encode to attributes
for i in range(10):
	an_int = i
	an_int_, an_int_batch, attr_A0_true, attr_A1_true, attr_A2_true, attr_A3_true, attr_A4_true, attr_A5_true, attr_A6_true  = data_loader.GetNextPermBatchOfRandomSize(permissible_list = [an_int])
	[attr_A0_, attr_A1_, attr_A2_, attr_A3_, attr_A4_, attr_A5_, attr_A6_] = sess.run([attr_A0, attr_A1, attr_A2, attr_A3, attr_A4, attr_A5, attr_A6], feed_dict = { X : an_int_batch })
	print('[TESTING] For the digit ' + str(an_int) + ', predicted attribute values are : ')
	print('[TESTING] 		Attribute A0 : ' + str(np.mean(attr_A0_)) + ' True : ' + str(attr_A0_true[0][0]))
	print('[TESTING] 		Attribute A1 : ' + str(np.mean(attr_A1_)) + ' True : ' + str(attr_A1_true[0][0]))
	print('[TESTING] 		Attribute A2 : ' + str(np.mean(attr_A2_)) + ' True : ' + str(attr_A2_true[0][0]))
	print('[TESTING] 		Attribute A3 : ' + str(np.mean(attr_A3_)) + ' True : ' + str(attr_A3_true[0][0]))
	print('[TESTING] 		Attribute A4 : ' + str(np.mean(attr_A4_)) + ' True : ' + str(attr_A4_true[0][0]))
	print('[TESTING] 		Attribute A5 : ' + str(np.mean(attr_A5_)) + ' True : ' + str(attr_A5_true[0][0]))
	print('[TESTING] 		Attribute A6 : ' + str(np.mean(attr_A6_)) + ' True : ' + str(attr_A6_true[0][0]))

...
SAM_W_logstd_A0 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A0')
SAM_B_logstd_A0 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A0')
DEC_W1_A0 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A0')
DEC_B1_A0 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A0')
DEC_W2_A0 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A0')
DEC_B2_A0 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A0')
ATT_W1_A0 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A0')
...
ENC_W1_A1 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A1')
ENC_B1_A1 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A1')
ENC_W2_A1 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A1')
ENC_B2_A1 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A1')
SAM_W_mu_A1 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A1')
SAM_B_mu_A1 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A1')
SAM_W_logstd_A1 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A1')
SAM_B_logstd_A1 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A1')
DEC_W1_A1 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A1')
DEC_B1_A1 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A1')
DEC_W2_A1 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A1')
DEC_B2_A1 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A1')
ATT_W1_A1 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A1')
...
ENC_W1_A2 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A2')
ENC_B1_A2 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A2')
ENC_W2_A2 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A2')
ENC_B2_A2 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A2')
SAM_W_mu_A2 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A2')
SAM_B_mu_A2 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A2')
SAM_W_logstd_A2 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A2')
SAM_B_logstd_A2 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A2')
DEC_W1_A2 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A2')
DEC_B1_A2 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A2')
DEC_W2_A2 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A2')
DEC_B2_A2 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A2')
ATT_W1_A2 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A2')
...
ENC_W1_A3 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A3')
ENC_B1_A3 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A3')
ENC_W2_A3 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A3')
ENC_B2_A3 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A3')
SAM_W_mu_A3 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A3')
SAM_B_mu_A3 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A3')
SAM_W_logstd_A3 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A3')
SAM_B_logstd_A3 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A3')
DEC_W1_A3 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A3')
DEC_B1_A3 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A3')
DEC_W2_A3 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A3')
DEC_B2_A3 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A3')
ATT_W1_A3 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A3')
...
ENC_W1_A4 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A4')
ENC_B1_A4 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A4')
ENC_W2_A4 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A4')
ENC_B2_A4 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A4')
SAM_W_mu_A4 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A4')
SAM_B_mu_A4 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A4')
SAM_W_logstd_A4 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A4')
SAM_B_logstd_A4 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A4')
DEC_W1_A4 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A4')
DEC_B1_A4 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A4')
DEC_W2_A4 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A4')
DEC_B2_A4 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A4')
ATT_W1_A4 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A4')
...
ENC_W1_A5 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A5')
ENC_B1_A5 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A5')
ENC_W2_A5 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A5')
ENC_B2_A5 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A5')
SAM_W_mu_A5 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A5')
SAM_B_mu_A5 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A5')
SAM_W_logstd_A5 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A5')
SAM_B_logstd_A5 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A5')
DEC_W1_A5 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A5')
DEC_B1_A5 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A5')
DEC_W2_A5 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A5')
DEC_B2_A5 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A5')
ATT_W1_A5 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A5')
...
ENC_W1_A6 = tf.get_variable(shape = [IMG_SIZE, ENC_DIM], stddev = 0.1), name = 'ENC_W1_A6')
ENC_B1_A6 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'ENC_B1_A6')
ENC_W2_A6 = tf.get_variable(shape = [ENC_DIM, LATENT_DIM], stddev = 0.1), name = 'ENC_W2_A6')
ENC_B2_A6 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), name = 'ENC_B2_A6')
SAM_W_mu_A6 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_mu_A6')
SAM_B_mu_A6 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_mu_A6')
SAM_W_logstd_A6 = tf.get_variable(shape = [LATENT_DIM, LATENT_DIM], stddev = 0.1), name = 'SAM_W_logstd_A6')
SAM_B_logstd_A6 = tf.get_variable(shape = [LATENT_DIM], stddev = 0.1), 'SAM_B_logstd_A6')
DEC_W1_A6 = tf.get_variable(shape = [LATENT_DIM, ENC_DIM], stddev = 0.1), name = 'DEC_W1_A6')
DEC_B1_A6 = tf.get_variable(shape = [ENC_DIM], stddev = 0.1), name = 'DEC_B1_A6')
DEC_W2_A6 = tf.get_variable(shape = [ENC_DIM, IMG_SIZE], stddev = 0.1), name = 'DEC_W2_A6')
DEC_B2_A6 = tf.get_variable(shape = [IMG_SIZE], stddev = 0.1), name = 'DEC_B2_A6')
ATT_W1_A6 = tf.get_variable(shape = [ENC_DIM, 1], stddev = 0.1), name = 'ATT_W1_A6')
